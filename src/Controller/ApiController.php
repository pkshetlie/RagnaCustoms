...
class ApiController extends AbstractController
{
    ...
    public function checkUpdates(Request $request, SongRepository $songRepository): Response
    {
        ...
        $songs = $songRepository->createQueryBuilder('s')
            ->select(
                's.id, s.name, s.authorName AS author, s.levelAuthorName AS mapper, s.newGuid AS hash, GROUP_CONCAT(r.level) AS Difficulties'
            )
    ...
    {
        $song = $songRepository->createQueryBuilder('s')->where('s.newGuid LIKE :search_string)')->andWhere(
            's.moderated = true'
        )->setParameter('search_string', $hash)->getQuery()->setFirstResult(0)->setMaxResults(1)->getOneOrNullResult();
        if (!$song) {
    ...
    {
        $data = $categoryRepository->createQueryBuilder("sc")->select("sc.id AS id, sc.label AS text")->where(
            'sc.label LIKE :search'
        )->setParameter('search', '%'.$request->get('q').'%')->andWhere('sc.isOnlyForAdmin = false')->orderBy(
            'sc.label'
        )->getQuery()->getArrayResult();
